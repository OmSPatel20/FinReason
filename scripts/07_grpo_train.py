"""Step 7: Stage 2 — GRPO Training with Verifiable Rewards.
The core of the project. Uses TRL's GRPOTrainer.
Time: ~2-4 hours on RTX 4060, faster on Colab A100.
"""
import torch, json, os, sys
from datasets import load_dataset, Dataset

sys.path.insert(0, os.path.dirname(__file__))
from data_utils import extract_qa, extract_context, build_prompt, SYSTEM_MSG, truncate_context
...
for i in range(min(MAX_TRAIN, len(train_raw))):
    ...
    if not a.strip():
        continue
    prompts.append(build_prompt(ctx, q))
    answers.append(a)
...
print(f"  Prompts: {len(prompts)} | Group size: {NUM_GENERATIONS}")
print(f"  LR: {LR} | Epochs: {GRPO_EPOCHS}")
...
logs = [l for l in trainer.state.log_history if any(k in l for k in ["loss","reward"])]
with open("outputs/grpo_training_log.json", "w") as f:
    json.dump(logs, f, indent=2)

print(f"\n✓ GRPO done. Saved to {OUTPUT_DIR}/final_adapter")
