"""
Step 5: Evaluate SFT model — Checkpoint 2 of 3.
"""
...
try:
    ...
    print("Loaded SFT model with Unsloth ✓")
except ImportError:
    ...
    model.eval()
    print("Loaded SFT model with PEFT ✓")
...
zs_path = "outputs/zeroshot_results.json"
if os.path.exists(zs_path):
    zs_acc = json.load(open(zs_path))["accuracy"]
    print(f"  Zero-shot was: {zs_acc*100:.2f}%  |  Δ = +{(acc-zs_acc)*100:.2f}%")

with open("outputs/sft_results.json", "w") as f:
    json.dump({"model": MODEL_NAME, "adapter": ADAPTER,
               "accuracy": acc, "n": len(predictions), "results": log}, f, indent=2)

if acc < 0.10:
    print("\n  ⚠ SFT accuracy < 10%. GRPO will struggle.")
    print("    Consider: more SFT epochs, or switch to larger model.")
else:
    print("\n  ✓ Ready for GRPO.")
